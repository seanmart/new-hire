<template>
  ...
  :class="{'bg-fuchsia-50 border-fuchsia-300 text-fuchsia-800':active}">
      ...
      <div class="mr-3 w-10 p-0 flex items-center justify-center relative">
        ...
        <transition name="item">
          ...
          <div v-else>
            ...
            <div class="h-8 w-8 border-2 border-shade-300 rounded" :class="{'hidden': active}">
...
<script>
  ...
  export default {
    ...
    created(){
      this.active = this.data.completed
    },
    methods:{
      async handleClick(){
        if (this.busy) return
