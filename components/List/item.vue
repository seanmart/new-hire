<template>
  <div 
  class="shadow px-3 py-2 min-h-4 mb-1 border-t border-b sm:border border-shade-200 sm:rounded flex items-center relative" 
  :class="{'bg-fuchsia-50 border-fuchsia-200 text-fuchsia-800':data.completed}">

    <div class="mr-3 w-12 p-0 flex-shrink-0 flex-grow-0 flex items-center justify-center relative cursor-pointer" @click="handleClick">
      ...
      <div v-else class="w-10 h-10 border-2 border-shade-300 rounded hover:border-shade-600"/>

    </div>

    <h5 class="xs:text-lg flex-auto mr-auto pointer-events-none" v-html="data.Task" />

    <a 
    :class="{'opacity-0 pointer-events-none': data.completed}"
    :href="data.Link" 
    target="_blank" 
    class="relative z-20 ml-5 button bg-pink text-white hover:bg-shade-600" 
    v-html="'Link'"/>
